...
if __name__ == '__main__':
    # 推荐使用 python example_ocr.py 作为入口
    import cv2
    model = ONNXPaddleOcr(use_angle_cls=True, use_gpu=False)
    img = cv2.imread('onnxocr/test_images/before_roi_155426.jpg')
    if img is not None:
        result = model.ocr(img)
        sav2Img(img, result)
        print("total time see example_ocr.py; result saved to draw_ocr.jpg")