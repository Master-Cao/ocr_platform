...
namespace template_matching {
	...
	// 辅助函数：检查点是否有效
	bool IsPointValid(const Point2f& pt) {
		return isfinite(pt.x) && isfinite(pt.y);
	}
	
	// 辅助函数：检查点是否有效
	bool IsPointValid(const Point2d& pt) {
		return isfinite(pt.x) && isfinite(pt.y);
	}
	
	// 辅助函数：安全的角度归一化
	double SafeNormalizeAngle(double angle) {
		if (!isfinite(angle))
			return 0.0;
	...
	// 辅助函数：安全的三角函数计算
	bool SafeTrigonometry(double angle, double& cosVal, double& sinVal) {
		if (!isfinite(angle))
			return false;
		...
		return isfinite(cosVal) && isfinite(sinVal);
	}
...
{
	// 防止NaN输入
	if (!isfinite(ptInput.x) || !isfinite(ptInput.y) || !isfinite(ptOrg.x) || !isfinite(ptOrg.y) || !isfinite(dAngle))
		return Point2f(0.0f, 0.0f);

	// 检查中心点是否为0，防止后续计算异常
	if (fabs(ptOrg.x) < DBL_EPSILON || fabs(ptOrg.y) < DBL_EPSILON)
		return Point2f(ptInput.x, ptInput.y); // 如果中心点为0，返回原始点

	double dWidth = static_cast<double>(ptOrg.x) * 2;
	double dHeight = static_cast<double>(ptOrg.y) * 2;
	double dY1 = dHeight - static_cast<double>(ptInput.y), dY2 = dHeight - static_cast<double>(ptOrg.y);

	// 检查三角函数计算的有效性
	double cosAngle = cos(dAngle);
	double sinAngle = sin(dAngle);
	if (!isfinite(cosAngle) || !isfinite(sinAngle))
		return Point2f(ptInput.x, ptInput.y); // 如果三角函数计算无效，返回原始点

	double dX = (static_cast<double>(ptInput.x) - static_cast<double>(ptOrg.x)) * cosAngle - (dY1 - static_cast<double>(ptOrg.y)) * sinAngle + static_cast<double>(ptOrg.x);
	double dY = (static_cast<double>(ptInput.x) - static_cast<double>(ptOrg.x)) * sinAngle + (dY1 - static_cast<double>(ptOrg.y)) * cosAngle + dY2;
...
*/
	...
	void CCOEFF_Denominator(cv::Mat& matSrc, s_TemplData* pTemplData, cv::Mat& matResult, int iLayer)
	{
		...
		// 输入验证
		if (matSrc.empty() || matResult.empty() || !pTemplData)
			return;
		...
		if (!isfinite(dTemplMean0) || !isfinite(dTemplNorm) || !isfinite(dInvArea) || dInvArea <= 0)
		{
			matResult = Scalar::all(0);
			return;
		}
	...
	{
		//比對到的區域需考慮重疊比例
	int iStartX = static_cast<int>(ptMaxLoc.x - static_cast<double>(sizeTemplate.width) * (1 - dMaxOverlap));
	int iStartY = static_cast<int>(ptMaxLoc.y - static_cast<double>(sizeTemplate.height) * (1 - dMaxOverlap));
	Rect rectIgnore(iStartX, iStartY, static_cast<int>(2 * static_cast<double>(sizeTemplate.width) * (1 - dMaxOverlap))
		, static_cast<int>(2 * static_cast<double>(sizeTemplate.height) * (1 - dMaxOverlap)));
		//塗黑
	...
	{
		// 输入验证
		if (!vec || vec->empty() || iMaxScoreIndex < 0 || iMaxScoreIndex >= vec->size())
			return false;
		if (!dNewX || !dNewY || !dNewAngle)
			return false;
		if (!isfinite(dAngleStep) || dAngleStep <= 0)
			return false;
		...
		// 检查输入坐标的有效性
		if (!isfinite(dX_maxScore) || !isfinite(dY_maxScore) || !isfinite(dTheata_maxScore))
			return false;
		...
		// 检查矩阵求逆是否成功
		if (matZ.empty() || matZ.rows == 0 || matZ.cols == 0)
			return false;
		...
		// 检查最终结果的有效性
		if (matDelta.empty() || matDelta.rows != 3 || matDelta.cols != 1)
			return false;
		
		double dResultX = matDelta.at<double>(0, 0);
		double dResultY = matDelta.at<double>(1, 0);
		double dResultAngle = matDelta.at<double>(2, 0) * R2D;
		
		// 验证输出结果的有效性
		if (!isfinite(dResultX) || !isfinite(dResultY) || !isfinite(dResultAngle))
			return false;

		*dNewX = dResultX;
		*dNewY = dResultY;
		*dNewAngle = dResultAngle;
		return true;
	}
	...
	int iSize = static_cast<int>(vec->size()), iIndexDelete = iSize + 1;
		for (int i = 0; i < iSize; i++)
		{
	...
	{
		int iSize = static_cast<int>(vecSort.size());
		Point2f ptCenter;
		for (int i = 0; i < iSize; i++)
			ptCenter += vecSort[i];
		ptCenter /= iSize;
	...
	int PatternMatcher::match(const cv::Mat& image, std::vector<MatchResult>& matchResults)
	{
		...
		int iTopLayer = GetTopLayer(&templateImage_, static_cast<int>(sqrt(static_cast<double>(matchParam_.minArea))));
		//建立金字塔
		...
		if (m_bDebugMode)
		{
			...
			for (int i = 0; i < iMatchSize; i++)
			{
				...
				ptRT = Point2f(ptLT.x + static_cast<float>(iDstW) * static_cast<float>(cos(dRAngle)), ptLT.y - static_cast<float>(iDstW) * static_cast<float>(sin(dRAngle)));
		ptLB = Point2f(ptLT.x + static_cast<float>(iDstH) * static_cast<float>(sin(dRAngle)), ptLT.y + static_cast<float>(iDstH) * static_cast<float>(cos(dRAngle)));
		ptRB = Point2f(ptRT.x + static_cast<float>(iDstH) * static_cast<float>(sin(dRAngle)), ptRT.y + static_cast<float>(iDstH) * static_cast<float>(cos(dRAngle)));
			...
			//for (int i = 0; i < iSearchSize; i++)
		{
			...
			if (!isfinite(dAngleStep) || dAngleStep <= 0.0)
				dAngleStep = 0.5;
					...
					{
						for (int i = -2; i <= 2; i++)
							vecAngles.push_back(dMatchedAngle + static_cast<double>(dAngleStep) * static_cast<double>(i));
					}
					else
					{
						if (matchParam_.angle < VISION_TOLERANCE)
					...
					Point2f ptPaddingLT = ptRotatePt2f(ptLT * 2, ptSrcCenter, static_cast<double>(dNewMatchAngle * D2R)) - Point2f(3, 3);
					Point2f pt(vecNewMatchParameter[iMaxScoreIndex].pt.x + ptPaddingLT.x, vecNewMatchParameter[iMaxScoreIndex].pt.y + ptPaddingLT.y);
					//再旋轉
					pt = ptRotatePt2f(pt, ptSrcCenter, static_cast<double>(-dNewMatchAngle * D2R));
		...
		FilterWithRotatedRect(&vecAllResult, TM_CCOEFF_NORMED, matchParam_.iouThreshold);
		//最後濾掉重疊
		...
		for (int i = 0; i < iMatchSize; i++)
		{
			...
			sstm.LeftTop = vecAllResult[i].pt;
				...
				// 验证输入点的有效性
				if (!isfinite(sstm.LeftTop.x) || !isfinite(sstm.LeftTop.y))
					continue;

				double cosVal = cos(dRAngle);
				double sinVal = sin(dRAngle);
				
				// 检查三角函数计算的有效性
				if (!isfinite(cosVal) || !isfinite(sinVal))
					continue;

				sstm.RightTop = Point2d(sstm.LeftTop.x + static_cast<double>(iW) * cosVal, sstm.LeftTop.y - static_cast<double>(iW) * sinVal);
				sstm.LeftBottom = Point2d(sstm.LeftTop.x + static_cast<double>(iH) * sinVal, sstm.LeftTop.y + static_cast<double>(iH) * cosVal);
				sstm.RightBottom = Point2d(sstm.RightTop.x + static_cast<double>(iH) * sinVal, sstm.RightTop.y + static_cast<double>(iH) * cosVal);
				
				// 验证计算结果的有效性
				if (!isfinite(sstm.RightTop.x) || !isfinite(sstm.RightTop.y) ||
					!isfinite(sstm.LeftBottom.x) || !isfinite(sstm.LeftBottom.y) ||
					!isfinite(sstm.RightBottom.x) || !isfinite(sstm.RightBottom.y))
					continue;
				...
				// 验证中心点的有效性
				if (!isfinite(sstm.Center.x) || !isfinite(sstm.Center.y))
					continue;
				sstm.Angle = -vecAllResult[i].dMatchAngle;
				sstm.Score = vecAllResult[i].dMatchScore;
				
				// 验证角度和分数的有效性
				if (!isfinite(sstm.Angle) || !isfinite(sstm.Score))
					continue;
				
				// 确保分数在合理范围内
				if (sstm.Score < 0.0) sstm.Score = 0.0;
				if (sstm.Score > 1.0) sstm.Score = 1.0;
				
				// 使用安全的角度归一化
				sstm.Angle = SafeNormalizeAngle(sstm.Angle);

			// 角度已经在SafeNormalizeAngle中处理，这里可以移除重复逻辑
			// 保留原始逻辑作为后备
			if (sstm.Angle < -180)
				sstm.Angle += 360;
			if (sstm.Angle > 180)
			...
			// 最终验证：确保所有坐标都是有效值
			if (!isfinite(sstm.LeftTop.x) || !isfinite(sstm.LeftTop.y) ||
				!isfinite(sstm.RightTop.x) || !isfinite(sstm.RightTop.y) ||
				!isfinite(sstm.LeftBottom.x) || !isfinite(sstm.LeftBottom.y) ||
				!isfinite(sstm.RightBottom.x) || !isfinite(sstm.RightBottom.y) ||
				!isfinite(sstm.Center.x) || !isfinite(sstm.Center.y))
			{
				continue; // 跳过无效的结果
			}
			...
			//Test Subpixel
				...
				if (matchResults.size() > 0 && 
					(!IsPointValid(matchResults.back().Center) || 
					 !IsPointValid(matchResults.back().LeftTop) ||
					 !IsPointValid(matchResults.back().RightTop) ||
					 !IsPointValid(matchResults.back().LeftBottom) ||
					 !IsPointValid(matchResults.back().RightBottom) ||
					 !isfinite(matchResults.back().Angle) ||
					 !isfinite(matchResults.back().Score)))
				{
					matchResults.pop_back();
				}
			if (i + 1 == matchParam_.maxCount)
				break;
		}

		return static_cast<int>(matchResults.size());
	}
