...
cv::Mat getRotateCropImage(const cv::Mat &src, std::vector<cv::Point> box) {
    ...
    if (left < 0 || top < 0 || right > src.cols || bottom > src.rows) {
        left = std::max(0, left);
        top = std::max(0, top);
        right = std::min(src.cols, right);
        bottom = std::min(src.rows, bottom);
    }
    ...
    // Python: img_crop_width = int(max(norm(p0-p1), norm(p2-p3)))
    //         img_crop_height = int(max(norm(p0-p3), norm(p1-p2)))
    double w1 = sqrt(pow(points[0].x - points[1].x, 2) + pow(points[0].y - points[1].y, 2));
    double w2 = sqrt(pow(points[2].x - points[3].x, 2) + pow(points[2].y - points[3].y, 2));
    int imgCropWidth = int(std::max(w1, w2));
    double h1 = sqrt(pow(points[0].x - points[3].x, 2) + pow(points[0].y - points[3].y, 2));
    double h2 = sqrt(pow(points[1].x - points[2].x, 2) + pow(points[1].y - points[2].y, 2));
    int imgCropHeight = int(std::max(h1, h2));
    ...
    cv::Mat partImg;
    // Python: flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE
    // 注意: 第5个参数是 flags（插值方式），第6个参数是 borderMode
    // 之前错误地将 BORDER_REPLICATE 传给了 flags 参数
...
std::vector<char *> getInputNames(Ort::Session *session) {
    ...
    for (size_t i = 0; i < numInputNodes; i++) {
        Ort::AllocatedStringPtr namePtr = session->GetInputNameAllocated(i, allocator);
        inputNodeNames[i] = my_strdup(namePtr.get());
    }
...
std::vector<char *> getOutputNames(Ort::Session *session) {
    ...
    for (size_t i = 0; i < numOutputNodes; i++) {
        Ort::AllocatedStringPtr namePtr = session->GetOutputNameAllocated(i, allocator);
        outputNodeNames[i] = my_strdup(namePtr.get());
    }
...
void getInputName(Ort::Session *session, char *&inputName) {
    ...
    if (numInputNodes > 0) {
        Ort::AllocatorWithDefaultOptions allocator;
        Ort::AllocatedStringPtr namePtr = session->GetInputNameAllocated(0, allocator);
        inputName = my_strdup(namePtr.get());
    }
}
...
void getOutputName(Ort::Session *session, char *&outputName) {
    ...
    if (numOutputNodes > 0) {
        Ort::AllocatorWithDefaultOptions allocator;
        Ort::AllocatedStringPtr namePtr = session->GetOutputNameAllocated(0, allocator);
        outputName = my_strdup(namePtr.get());
    }
}

void saveImg(cv::Mat &img, const char *imgPath) {
    if (img.empty() || imgPath == nullptr || strlen(imgPath) == 0)
        return;
    if (!cv::imwrite(imgPath, img))
        fprintf(stderr, "保存图像失败: %s\n", imgPath);
}
